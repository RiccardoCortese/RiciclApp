...
def clean(s):
    # normalizza i trattini lunghi per leggibilita'
    return s.replace('—', '-').replace('–', '-')


# Raccogli righe di dettaglio + statistiche per suite
rows = []
per_suite = {}
...
ws['A1'] = 'RiciclApp - Risultati Test Backend'
ws['A1'].font = TITLE_FONT
ws['A2'] = f'Black-box API testing (Jest + Supertest) - Eseguito il {run_dt}'
ws['A2'].font = SUB_FONT
...
# Tabella per-suite
r += 1
ws.cell(r, 1, 'Dettaglio per suite').font = Font(name=FONT, bold=True, size=12, color='1B5E20')
r += 1
...
totrow = ['TOTALE', passed, failed, total, tot_dur]
for j, v in enumerate(totrow, 1):
    c = ws.cell(r, j, v)
    c.font = BOLD
    c.border = BORDER
    c.alignment = LEFT if j == 1 else CENTER

for col, w in zip('ABCDE', [34, 12, 10, 10, 13]):
    ws.column_dimensions[col].width = w

# ── Foglio 2: Dettaglio Test ─────────────────────────────────────────────────
ws2 = wb.create_sheet('Dettaglio Test')
ws2.sheet_view.showGridLines = False
heads2 = ['#', 'Suite', 'Gruppo / Endpoint', 'Test case', 'Esito', 'Durata (ms)']
...
for col, w in zip('ABCDEF', [5, 16, 40, 60, 9, 12]):
    ws2.column_dimensions[col].width = w

out = 'Results_Backend.xlsx'
wb.save(out)
